...
void eval(char *cmdline)
{
  ...
  if (builtin_cmd(argv) == 0) {
    ...
    // block sigchld signal for addjob correctly
    sigset_t mask;
    sigemptyset(&mask);
	...
	unix_error("addjob failed");
      ...
      sigprocmask(SIG_UNBLOCK, &mask, NULL);
      if (!bg) {
	// waiting for the forground job to terminate
	waitfg(pid);
      } else {
...
void do_bgfg(char **argv)
{
  ...
  if (strcmp(cmd, "bg") == 0) {
    ...
    unix_error("invalid command for do_bgfg");
  }
}
